...
class Basic
{
    ...
    {
        if(strpos($className,'PhpOffice') !== false || strpos($className,'SimpleCache') !== false || strpos($className,'Qiniu') !== false || strpos($className,'OSS') !== false){
            $class_file = CORE_PATH . '/extend/' . str_replace('\\', '/', $className) . '.php';
        } elseif (substr($className, 0, 4) == 'core') { // 框架类文件命名空间转换
