...
function upload($input_name, $file_ext = null, $max_width = null, $max_height = null, $watermark = false)
{
    ...
    if (is_array($files['tmp_name'])) { // 多文件情况
        ...
        for ($i = 0; $i < $file_count; $i ++) {
            if (! $files['error'][$i]) {
                //存储文件
                if( Config::get('qiniu_open') ){
                    $upfile = qiniuSave($files['tmp_name'][$i],$files['name'][$i]);
                }elseif( Config::get('oss_open') ){
                        $upfile = ossSave($files['tmp_name'][$i],$files['name'][$i]);
                }else{
        ...
        if (! $files['error']) {
            if( Config::get('qiniu_open') ){
                $upfile = qiniuSave($files['tmp_name'],$files['name']);
            }elseif( Config::get('oss_open') ){
                $upfile = ossSave($files['tmp_name'],$files['name']);
            }else{
...
//七牛存储
function qiniuSave($srcFileName, $destFileName, $stream=false) {
    $file = explode('.', $destFileName); // 分离文件名及扩展
    $file_ext = strtolower(end($file)); // 获取扩展
    $filename = date('Ymd') . '/' . time() . mt_rand(100000, 999999) . '.' . $file_ext; //新的文件名
    $qiniu = new QiniuStorage();
    $result = $qiniu->saveFile($srcFileName, $filename, $stream);
    if ( 1== $result['code']) {
        return $result['msg'];
    } else {
        return $result['msg'];
...
//阿里云OSS
function ossSave($srcFileName, $destFileName, $stream=false) {
    $file = explode('.', $destFileName); // 分离文件名及扩展
    $file_ext = strtolower(end($file)); // 获取扩展
    $filename = date('Ymd') . '/' . time() . mt_rand(100000, 999999) . '.' . $file_ext; //新的文件名
    $oss = new OssStorage();
    $result = $oss->saveFile($srcFileName, $filename, $stream);
    if ( 1== $result['code']) {
        return $result['msg'];
    } else {
        return $result['msg'];
